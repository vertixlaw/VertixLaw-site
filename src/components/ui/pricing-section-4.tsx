...
const plans = [
  {
    ...
    includes: [
      "Inclui:",
      "2 Contratos Mensais de qualquer natureza",
      "Contrato social",
      "Acordo de sócios",
      "NDA básico",
      "Suporte por email",
...
const PricingSwitch = ({ onSwitch }: { onSwitch: (value: string) => void }) => {
  const [selected, setSelected] = useState("0"); // Voltando para o padrão
    ...
    <div className="flex justify-center">
      <div className="relative z-10 mx-auto flex w-fit rounded-full bg-neutral-900 border border-gray-700 p-1">
        ...
        >
          ...
          <span className="relative">Único</span>
        </button>
        ...
        <button
          ...
          className={cn(
            "relative z-10 w-fit h-10 flex-shrink-0 rounded-full sm:px-6 px-3 sm:py-2 py-1 font-medium transition-colors",
            selected === "2" ? "text-white" : "text-gray-200",
          )}
        >
          {selected === "2" && (
            <motion.span
...
export default function PricingSection4() {
  const [pricingMode, setPricingMode] = useState("0"); // 0: Único, 1: Mensal, 2: Anual (iniciando com Único)
  ...
  // Plano único
  const uniquePlan = {
    name: "Consultoria Única",
    description: "Solução pontual para suas necessidades jurídicas específicas",
    price: 189,
    buttonText: "Converse com nossa equipe",
    buttonVariant: "outline" as const,
    includes: [
    ...
    <section id="planos" className="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 max-w-5xl gap-6 mx-auto">
          {pricingMode === "0" ? (
            // Mostrar apenas o plano único centralizado
            <div className="md:col-start-2">
              <Card className="relative transform transition-all duration-300 hover:scale-105 hover:shadow-xl border-gray-200 hover:border-blue-300">
                <CardHeader className="text-left">
                  <div className="flex justify-between">
                    <h3 className="text-3xl mb-2 text-gray-900">{uniquePlan.name}</h3>
                  </div>
                  ...
                  <p className="text-sm text-gray-600 mb-4">{uniquePlan.description}</p>
                </CardHeader>

                <CardContent className="pt-0">
                  <button
                    onClick={scrollToContact}
                    className="w-full mb-6 p-4 text-sm rounded-xl font-medium transition-all duration-300 transform hover:scale-105 hover:shadow-xl bg-white text-gray-900 border-2 border-gray-300 hover:border-blue-400 hover:bg-blue-50 hover:text-blue-700"
                  >
                    {uniquePlan.buttonText}
                  </button>

                  <div className="space-y-3 pt-4 border-t border-gray-200">
                    <h4 className="font-medium text-base mb-3 text-gray-900">
                      {uniquePlan.includes[0]}
                    </h4>
                    <ul className="space-y-2">
                      {uniquePlan.includes.slice(1).map((feature, featureIndex) => (
                        <li
                          key={featureIndex}
            ...
            // Mostrar os planos mensais/anuais
                  ...
                  <div className="flex items-baseline">
                    <span className="text-4xl font-semibold text-gray-900">
                      ...
                      <NumberFlow
                        ...
                        value={pricingMode === "2" ? plan.yearlyPrice : plan.price}
                        className="text-4xl font-semibold"
                      />
                    </span>
                    <span className="text-gray-500 ml-1">
                      /{pricingMode === "2" ? "ano" : "mês"}
                    </span>
                  </div>
                  {pricingMode === "2" && (
